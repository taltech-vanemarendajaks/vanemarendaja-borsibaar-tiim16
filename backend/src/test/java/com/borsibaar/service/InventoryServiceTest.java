...
class InventoryServiceTest {
    ...
    void addStock_CreatesInventoryIfMissing() {
        ...
        when(inventoryMapper.toResponse(any())).thenAnswer(inv -> {
            ...
            return new InventoryResponseDto(
                i.getId(), 
                i.getProduct().getOrganizationId(), 
                i.getProductId(), 
                "P", 
                i.getQuantity(), 
                i.getAdjustedPrice(), 
    ...
    void removeStock_Insufficient_ThrowsBadRequest() {
        ...
        Inventory inv = new Inventory(); inv.setId(9L); inv.getProduct().setOrganizationId(1L); inv.setProduct(product); inv.setProductId(5L); inv.setQuantity(BigDecimal.valueOf(2)); inv.setAdjustedPrice(BigDecimal.ONE); inv.setUpdatedAt(OffsetDateTime.now());
        when(productRepository.findById(5L)).thenReturn(Optional.of(product));
    ...
    void adjustStock_Success_CreatesTransaction() {
        Product product = new Product(); product.setId(5L); product.setOrganizationId(1L); product.setActive(true); product.setBasePrice(BigDecimal.valueOf(2));
        Inventory inv = new Inventory(); inv.setId(9L); inv.getProduct().setOrganizationId(1L); inv.setProduct(product); inv.setProductId(5L); inv.setQuantity(BigDecimal.valueOf(5)); inv.setAdjustedPrice(BigDecimal.valueOf(2)); inv.setUpdatedAt(OffsetDateTime.now());
        when(productRepository.findById(5L)).thenReturn(Optional.of(product));
        when(inventoryRepository.findByOrganizationIdAndProductId(1L, 5L)).thenReturn(Optional.of(inv));
        when(inventoryRepository.save(any(Inventory.class)))
                .thenAnswer(invocation -> invocation.getArgument(0));
        when(inventoryMapper.toResponse(any())).thenAnswer(a -> {
            Inventory i = a.getArgument(0); return new InventoryResponseDto(i.getId(), i.getProduct().getOrganizationId(), i.getProductId(), "Prod", i.getQuantity(), i.getAdjustedPrice(), product.getDescription(), null, null, null, i.getUpdatedAt().toString()); });
    ...
    void getByOrganization_FiltersInactiveProducts() {
        Inventory inv1 = new Inventory(); inv1.setId(1L); inv1.getProduct().setOrganizationId(1L); inv1.setProductId(10L); inv1.setQuantity(BigDecimal.ONE); inv1.setUpdatedAt(OffsetDateTime.now());
        Inventory inv2 = new Inventory(); inv2.setId(2L); inv2.getProduct().setOrganizationId(1L); inv2.setProductId(11L); inv2.setQuantity(BigDecimal.ONE); inv2.setUpdatedAt(OffsetDateTime.now());
    ...
    void getByProductAndOrganization_ProductInactive_Gone() {
        Inventory inv = new Inventory(); inv.setId(1L); inv.getProduct().setOrganizationId(1L); inv.setProductId(10L); inv.setQuantity(BigDecimal.ONE); inv.setUpdatedAt(OffsetDateTime.now());
    ...
    void removeStock_Success_CreatesNegativeTransaction() {
        ...
        Inventory inv = new Inventory(); inv.setId(10L); inv.getProduct().setOrganizationId(1L); inv.setProduct(product); inv.setProductId(5L); inv.setQuantity(new BigDecimal("10")); inv.setAdjustedPrice(new BigDecimal("2.00")); inv.setUpdatedAt(OffsetDateTime.now());
        when(productRepository.findById(5L)).thenReturn(Optional.of(product));
        when(inventoryRepository.findByOrganizationIdAndProductId(1L, 5L)).thenReturn(Optional.of(inv));
        when(inventoryRepository.save(any(Inventory.class))).thenAnswer(a -> a.getArgument(0));
        when(inventoryMapper.toResponse(any())).thenAnswer(a -> { Inventory i = a.getArgument(0); return new InventoryResponseDto(i.getId(), i.getProduct().getOrganizationId(), i.getProductId(), "Prod", i.getQuantity(), i.getAdjustedPrice(), product.getDescription(), null, null, null, i.getUpdatedAt().toString());});
    ...
    void getTransactionHistory_MapsUserInfo() {
        Inventory inv = new Inventory(); inv.setId(100L); inv.getProduct().setOrganizationId(1L); inv.setProductId(10L);
        when(inventoryRepository.findByOrganizationIdAndProductId(1L, 10L)).thenReturn(Optional.of(inv));
