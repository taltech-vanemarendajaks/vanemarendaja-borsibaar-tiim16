...
class SalesServiceTest {
    ...
    void processSale_SingleItem_SuccessPriceIncreaseCapped() {
        ...
        Inventory inventory = new Inventory(); inventory.setId(9L); inventory.setProduct(product); inventory.setProductId(5L); inventory.getProduct().setOrganizationId(1L); inventory.setQuantity(BigDecimal.valueOf(20)); inventory.setAdjustedPrice(BigDecimal.valueOf(10)); inventory.setUpdatedAt(OffsetDateTime.now());
    ...
    void processSale_InsufficientStock_Throws() {
        ...
        Inventory inventory = new Inventory(); inventory.setId(9L); inventory.setProduct(product); inventory.setProductId(5L); inventory.getProduct().setOrganizationId(1L); inventory.setQuantity(BigDecimal.ONE); inventory.setAdjustedPrice(BigDecimal.ONE);
        product.setInventory(inventory);
        when(productRepository.findById(5L)).thenReturn(Optional.of(product));
    ...
    void processSale_ProductOrgMismatch_Throws() {
        ...
        Inventory inventory = new Inventory(); inventory.setId(9L); inventory.setProduct(product); inventory.setProductId(5L); inventory.getProduct().setOrganizationId(2L); inventory.setQuantity(BigDecimal.ONE); inventory.setAdjustedPrice(BigDecimal.ONE);
        product.setInventory(inventory);
        when(productRepository.findById(5L)).thenReturn(Optional.of(product));
        SaleItemRequestDto item = new SaleItemRequestDto(5L, BigDecimal.ONE);
        SaleRequestDto request = new SaleRequestDto(List.of(item), null, null);
